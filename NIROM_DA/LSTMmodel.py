...
def createLSTM(lrate = 0.001, num_latent=8, windowSize=5, nBlocks = 3, **kwargs):
    ...
    for k in range(nBlocks):
        x = LSTM(numLayer[0], return_sequences=True,activation=act_dict[n_act], kernel_initializer=init_dict[n_init])(a) # main1 
        for i in range(1, len(numLayer)-1):
            x = LSTM(numLayer[i], return_sequences=True,activation=act_dict[n_act], kernel_initializer=init_dict[n_init])(x) # main1 
        for i in range(1, len(numLayerA)):
            a = LSTM(numLayerA[i], return_sequences=True,activation=act_dict[n_act], kernel_initializer=init_dict[n_init])(a) # skip1            
...
def createLSTMSeq(lrate = 0.001, num_latent=8, *args):
